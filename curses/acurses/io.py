...
def align_style_print(win: _curses.window, line: int, align: Literal[1, 2, 3], text: str) -> None:
    ...
    assert len(raw_text) <= width

    if align == 1: # left-align
        print_styled_mu(win, line, 0, text) # directly on left
    elif align == 3: # right-align
        print_styled_mu(win, line, width - len(raw_text), text)
    else: # centered
        print_styled_mu(win, line, (width - len(raw_text)) // 2, text)
